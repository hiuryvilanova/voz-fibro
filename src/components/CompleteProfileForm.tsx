...
export function CompleteProfileForm({ name }: { name: string }) {
  ...
  const [form, setForm] = useState({ name, state: "", city: "", phone: "", profession: "", cpf: "", acceptTerms: false });
  ...
  return <form onSubmit={submit} className="mt-8 space-y-5 rounded-lg border border-border bg-white p-5 shadow-sm sm:p-7">
    ...
    <div className="grid gap-4 sm:grid-cols-2"><div><label className="text-sm font-bold" htmlFor="complete-state">Estado</label><select id="complete-state" required value={form.state} onChange={(e) => { e.currentTarget.setCustomValidity(""); setForm({ ...form, state: e.target.value }); }} onInvalid={(e) => e.currentTarget.setCustomValidity("Selecione seu estado para continuar.")} className={inputClass}><option value="">Selecione</option>{ESTADOS.map((uf) => <option key={uf}>{uf}</option>)}</select></div><div><label className="text-sm font-bold" htmlFor="complete-city">Cidade</label><input id="complete-city" required value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} className={inputClass} /></div></div>
